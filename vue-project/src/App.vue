...
<template>
  ...
  <button v-for="key,value in pages" :class='{active:page === key}' @click="page = key">{{ value }}</button>
...
<section v-show="page === 1">
  ...
  <div>
    <p><label for="twd">台幣：<input type="text" name="twd" id="twd" v-model="twd"></label>元</p>
    <p><label for="jpy">日幣：<input type="text" name="jpy" id="jpy" v-model="jpy"></label>元</p>
    <p><label for="usd">美幣：<input type="text" name="usd" id="usd" v-model="usd"></label>元</p>
  </div>
